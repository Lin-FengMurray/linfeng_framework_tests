...
// "automationexercise/tests/..." → "automationexercise"
function detectSuite(filePath) {
  // Full path variant (absolute or relative from repo root)
  const m1 = filePath.match(/linfeng_framework_tests\/([^/]+)\/tests\//);
  if (m1) return m1[1];
  // Relative path variant (relative to testDir)
