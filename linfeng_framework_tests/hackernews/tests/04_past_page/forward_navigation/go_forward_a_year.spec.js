...
test.describe('Past Page - Go Forward a Year', () => {

  test('navigating forward a year changes the date in the URL', async ({ page }) => {
    await page.goto('https://news.ycombinator.com/front');
    const initialDate = await getCurrentDateFromUrl(page);
    const clicked = await clickIfExists(page, 'year', 1);

    if (clicked) {
      const newDate = await getCurrentDateFromUrl(page);
      expect(newDate).not.toBe(initialDate);
    } else {
      console.log('Forward year link does not exist (already at latest year)');
    }
  });
});
